...
void Renderer::InitVert() {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * this->mesh->vertices.size(), &this->mesh->vertices[0], GL_STATIC_DRAW); // 传入顶点数据到VBO
    // glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(int) * this->mesh->indices.size(), &this->mesh->indices[0], GL_STATIC_DRAW); // 传入索引数据到EBO
